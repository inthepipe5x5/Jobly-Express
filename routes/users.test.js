...
describe("POST /users/:username/jobs/:id", () => {
  ...
  beforeAll(async () => {
    const result = await db.query(`SELECT id FROM jobs WHERE title = $1`, [
      "job_1",
    ]);
    jobId = result.rows[0].id;
  });

  test("successfully apply for a job", async () => {
  ...
  test("duplicate job application returns error", async () => {
    await User.applyForJob("u1", jobId); // First application
